...
- 日期：2026-09-16
- 状态：设计已确认；计划一（颜色收口）已编写，计划二（亮色模式）待计划一合并后编写。规划中的修订见文末「规划阶段修订」
...
编写实现计划时在演练 worktree（基点 `6057baa8`）中实跑了全部改造，以下与上文不同之处以本节为准。

### 拆成两份计划

- **计划一：颜色收口**（`docs/superpowers/plans/2026-09-16-theme-color-tokens.md`）——token、四个 codemod、两个守护测试。可独立合并，不引入任何亮色外观。
- **计划二：亮色模式**——模式层、亮色调色板、JS 渲染器、启动闪屏、选择器 UI。依赖计划一的 token 与白名单，计划一合并后编写。

### 暗色约束的精确表述

上文「暗色模式像素级不变」修正为两条可验证的约束：

1. **暗色 `:root` 调色板字节级不变**：由金标快照测试强制，只允许新增 `--contrast`、`--shade`、`--shadow-popover`。
2. **白色叠加的编译产物逐条等价**：基线 49 条白色叠加规则中 46 条与改造后的 `contrast` 规则 `color-mix` 声明逐字相同，3 条按设计保留白色。

中性 hex 的**角色映射**（下一节）不满足像素级不变：Atlas Black 下 115 处中 84 处偏移不超过 4 个通道级，31 处移动更多（最大为 `#ccc`→`--text-primary`，差 51）。清单见计划一。此为用户选择「按角色归到语义 token」时接受的代价；其余暗色主题中这些表面开始跟随主题，原本不跟随属于现存缺陷。

### 新增：中性 hex 的角色映射

设计阶段漏算了约 180 处写死的深色 hex（`bg-[#1a1a1a]`、`text-[#555]`、纯 `bg-black` 等），它们在亮色下会成为散落的深色块。按角色映射到现有 token，不为每种灰新建 token。两条教训写进了 codemod：

- **静息与聚焦边框必须映射到不同 token**（`#303030`→`--border-default`，`#4a4a4a`→`--border-focus`）。映射到同一个会让聚焦不再可见。
- `text-text-muted` 在代码中出现 22 次但**不是生成的工具类**（`@theme` 无 `--color-text-muted`，构建产物中该类出现 0 次）。映射目标因此写作 `text-[var(--text-muted)]`。那 22 处现存用法实际继承父元素颜色，是独立缺陷，修复会改变暗色外观，不在本工作范围。

### 阴影：`--shade` 取代倍率

上文「阴影收进 `--shadow-*` token，亮色底把强度降到约 0.1」改为：

- 新增 `--shade` 颜色（暗色 `#000`，亮色为半透明黑），组件里的黑色阴影与凹陷带写作 `color-mix(in srgb, var(--shade) a%, transparent)`。亮色下 `--shade` 本身半透明，所有阴影自动按比例变淡，无需 `calc` 倍率。
- 12 个弹出菜单逐字相同的阴影收进 `--shadow-popover`。
- 模态遮罩（`bg-black/N` 于 overlay）两种模式都保持黑色。
- `:root` 中已有的 `--shadow-*` 保持字面量（快照约束），由亮色块覆盖。

### JS 渲染器：按模式各备一套调色板

上文「提升 `readToken()` + 派发 `atlas:theme-changed` 事件」改为：xterm、recharts、pixi 图谱、canvas、SVG 属性等在 TS 中按模式各备一套调色板，随模式切换重建。理由：这些值多数不是现有 token 能表达的（ANSI 16 色、图谱节点三档亮度），读 CSS 变量只是把字面量挪进 CSS 再读回来。另发现块渲染器 `line-emulator.ts` 有第二份 ANSI 16 色调色板，必须与 xterm 同步。

### 固定不随主题的组件

以下经核实保持字面量，登记在守卫测试白名单中：提示键帽（深色毛玻璃，三个文件）、标题栏 session-capture 蓝色胶囊、Windows 关闭按钮红、彩色填充上的白字（头像、危险按钮、协作光标、连接手柄）、PDF 页面、媒体黑边、开关旋钮。终端背景在计划一中保持 `#000`，因为它必须与 xterm 画布一致，计划二一并处理。

### 顺带修复

日志面板过滤勾选框的勾写作 `text-white`，画在 Atlas Black 的白色 accent 填充上，不可见。改为 `text-text-inverse`（`git-error-dialog.tsx` 注释中已记录此约定）。这是计划一中唯一有意的暗色外观修复。

### 执行环境发现（Windows）

- 工作区为 CRLF（`autocrlf=true`），`oxfmt --check` 对所有文件报假阳性；只用 `--write`。
- 演练中 codemod 改过的 14 个文件在基点上格式正确、改后超出行宽，故每个 codemod 之后显式运行 `oxfmt --write`。
