 ...
 * the sidebar row.
 *
 * A project stores the icon as a stable STRING key (`Workspace.icon`) rather
...
export interface ProjectIconDef {
  /** Stable key persisted on `Workspace.icon`. */
  key: string;
  Icon: LucideIcon;
...
/** `Workspace.icon` key → glyph. Built once from the list above. */
...
export interface ProjectColorDef {
  /** Stable key persisted on `Workspace.color`. */
  key: string;
