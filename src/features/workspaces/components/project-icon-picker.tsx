    ...
    >
      <Popover.Trigger asChild>
        <button
          ...
          className="flex size-9 shrink-0 items-center justify-center rounded-lg border border-[var(--border-default)] bg-[var(--bg-input)] transition-colors hover:border-[var(--border-strong)] cursor-pointer"
        >
          <ProjectGlyph icon={icon} color={color} size={16} muted="text-[var(--text-secondary)]" />
        </button>
      </Popover.Trigger>
      <Popover.Portal>
        <Popover.Content
          align="start"
          sideOffset={6}
          className="z-[var(--z-max)] w-[292px] rounded-xl border border-[var(--border-default)] bg-[var(--bg-overlay)] p-2 shadow-[var(--shadow-overlay)]"
        >
          {/* Tabs */}
          <div className="mb-2 flex gap-0.5 rounded-lg bg-[var(--bg-input)] p-0.5">
            {(["icons", "emoji"] as const).map((t) => (
              <button
                ...
                onClick={() => setTab(t)}
                className={cn(
                  "flex-1 rounded-md px-2 py-1 text-[11px] capitalize transition-colors cursor-pointer",
                  tab === t
          ...
          {/* Search */}
          <div className="mb-2 flex h-7 items-center gap-1.5 rounded-md border border-[var(--border-default)] bg-[var(--bg-input)] px-2">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    !c.value && "border border-[var(--border-strong)]",
                  )}
                ...
                >
                  {selected && (
                    <Check
                      size={11}
                      strokeWidth={3}
                      className={c.value ? "text-white" : "text-[var(--text-secondary)]"}
                    />
                  )}
                </button>
              );
          ...
          <div className="hide-scrollbar grid max-h-[176px] grid-cols-6 gap-0.5 overflow-y-auto">
              ...
              icons.map(({ key, Icon }) => (
                <button
                  ...
                  className={cn(
                    ...
                    icon === key ? "bg-[var(--bg-active)]" : "hover:bg-[var(--bg-hover)]",
                  )}
                >
                  <Icon
                    size={14}
                    style={color ? { color } : undefined}
                    className={cn(!color && "text-[var(--text-secondary)]")}
                  />
                </button>
              ))}
            {tab === "emoji" &&
              emojis.map(({ char }) => (
                <button
                  ...
                  className={cn(
                    "flex size-8 items-center justify-center rounded-md text-[16px] leading-none transition-colors cursor-pointer",
                    icon === char ? "bg-[var(--bg-active)]" : "hover:bg-[var(--bg-hover)]",
                  )}
                >
                  {char}
                </button>
            ...
            {(tab === "icons" ? icons : emojis).length === 0 && (
              <div className="col-span-6 py-4 text-center text-[10px] text-[var(--text-tertiary)]">
                No matches
              </div>
            )}
          </div>
          ...
          <div className="mt-2 flex items-center justify-between border-t border-[var(--border-subtle)] pt-2">
            <span className="px-0.5 text-[10px] text-[var(--text-tertiary)]">
            ...
            {icon && (
              <button
                ...
                className="rounded px-1.5 py-0.5 text-[10px] text-[var(--text-tertiary)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-secondary)] cursor-pointer"
              >
                Remove icon
              </button>
            )}
